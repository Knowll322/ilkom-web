window.addEventListener('load', () => {
  ...
  if (heroTitle && !isTouchDevice) {
    ...
    lines.forEach(line => {
      const textContainers = line.querySelectorAll('span[class^="word-"]');
    ...
    const pressureChars = document.querySelectorAll('.pressure-char');
    const heroSection = document.getElementById('hero');
    let pmouseX = 0;
    let pmouseY = 0;
    let pIsHovering = false;
    let pRafId = null;
    ...
    heroSection.addEventListener('mousemove', (e) => {
      ...
      pIsHovering = true;
      if (!pRafId) {
        pRafId = requestAnimationFrame(updatePressure);
      }
    });

    heroSection.addEventListener('mouseleave', () => {
      pIsHovering = false;
      pressureChars.forEach(char => {
        char.style.fontWeight = '900';
    ...
    function updatePressure() {
      if (!pIsHovering) return;
      pressureChars.forEach(char => {
        const rect = char.getBoundingClientRect();
        const charCX = rect.left + rect.width / 2;
        const charCY = rect.top + rect.height / 2;
        const dx = pmouseX - charCX;
        const dy = pmouseY - charCY;
        const dist = Math.sqrt(dx * dx + dy * dy);
        
        const maxDist = 300;
        if (dist < maxDist) {
          const intensity = 1 - (dist / maxDist);
          const weight = Math.floor(400 + (intensity * 500));
          const spacing = intensity * 4;
          
          char.style.fontWeight = weight;
          char.style.letterSpacing = `${spacing}px`;
        } else {
  ...
  // LIGHTFALL EFFECT
  const lightfallCanvas = document.getElementById('lightfallCanvas');
  if (lightfallCanvas) {
    const lCtx = lightfallCanvas.getContext('2d');
    let lW, lH;
    const lParticles = [];
    
    const resizeLightfall = () => {
      lW = lightfallCanvas.width = lightfallCanvas.offsetWidth;
      lH = lightfallCanvas.height = lightfallCanvas.offsetHeight;
    };
    resizeLightfall();
    window.addEventListener('resize', resizeLightfall);
    
    const lColors = ['#FFB800', '#ffffff', '#60A5FA'];
    for (let i = 0; i < 100; i++) {
      lParticles.push({
        x: Math.random() * lW,
        y: Math.random() * lH,
        ...
        color: lColors[Math.floor(Math.random() * lColors.length)],
    ...
    function drawLightfall() {
      ...
      lParticles.forEach(p => {
        p.y += p.speed;
        p.angle += 0.02;
        const currentX = p.x + Math.sin(p.angle) * p.swing;
        ...
        lCtx.beginPath();
        lCtx.arc(currentX, p.y, p.size, 0, Math.PI * 2);
        lCtx.fillStyle = p.color;
        lCtx.globalAlpha = p.opacity;
        lCtx.shadowBlur = p.size * 2;
        lCtx.shadowColor = p.color;
        lCtx.fill();
        lCtx.shadowBlur = 0;
      });
      lCtx.globalAlpha = 1;
      requestAnimationFrame(drawLightfall);
    }
    drawLightfall();
  }

  // SCROLL STACK EFFECT
  const aboutBentoGrid = document.querySelector('#about .bento-grid');
  const aboutBentoCards = document.querySelectorAll('#about .bento-card');
  if (aboutBentoGrid && aboutBentoCards.length > 0) {
    aboutBentoGrid.classList.add('scroll-stack-container');
    aboutBentoCards.forEach((card, i) => {
      card.classList.add('scroll-stack-item');
      card.style.zIndex = i + 1;
    });

    const stackCards = () => {
      if (window.innerWidth <= 1024) {
        aboutBentoCards.forEach(c => c.style.transform = 'none');
        return;
      }
      
      aboutBentoCards.forEach((card, i) => {
        const rect = card.getBoundingClientRect();
        let stuckCountBelow = 0;
        
        for (let j = i + 1; j < aboutBentoCards.length; j++) {
          if (aboutBentoCards[j].getBoundingClientRect().top <= 105) {
            stuckCountBelow++;
          }
        }
        
        if (rect.top <= 105) {
          const scale = Math.max(0.8, 1 - (stuckCountBelow * 0.02));
